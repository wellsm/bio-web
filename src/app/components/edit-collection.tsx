...
export function EditCollection({
  ...
  const [selected, setSelected] = useState<ICollectionLink[]>(collection.links);
  ...
  const form = useForm<z.infer<typeof collectionSchema>>({
    ...
    defaultValues: {
      ...
      links: collection.links.map((link) => link.id),
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof collectionSchema>) => {
    ...
    try {
      await http.put(`collections/${collection.id}`, values);
    ...
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent className="overflow-y-auto scrollbar-thin dark:scrollbar-thumb-gray-50 dark:scrollbar-track-zinc-900">
        <SheetHeader>
          ...
          <SheetDescription>
            {t("Edit your collection here. Click save when you're done.")}
          </SheetDescription>
        </SheetHeader>
